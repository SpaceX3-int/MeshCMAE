...
from model.meshmae import Mesh_mae, Mesh_cmae
...
from model.mambamae import Mesh_mae_mamba
...
def train(net, optim, scheduler, names, train_dataset, epoch, args):
    ...
    for it, (feats_patch, center_patch,coordinate_patch, face_patch,  np_Fs, label, mesh_paths, feats_patch2, center_patch2, coordinate_patch2, face_patch2,  np_Fs2) in enumerate(  
            train_dataset):
        optim.zero_grad()
        ...
        faces2 = face_patch2.to(torch.float32).cuda()
        feats2 = feats_patch2.to(torch.float32).cuda()
        centers2 = center_patch2.to(torch.float32).cuda()
        Fs2 = np_Fs2.cuda()
        cordinates2 = coordinate_patch2.to(torch.float32).cuda()
        
        n_samples += faces.shape[0]
        #def forward(self, faces, feats, centers, Fs, cordinates, faces2, feats2, centers2, Fs2, cordinates2):
        loss = net(faces, feats, centers, Fs, cordinates, faces2, feats2, centers2, Fs2, cordinates2)
        loss.backward()
        optim.step()
    ...
    scheduler.step()
    if train.best_loss > epoch_loss and epoch_loss > 0:
...
def test(net,  names, test_dataset, epoch, args):
    ...
    for it, (feats_patch, center_patch, coordinate_patch, face_patch, np_Fs, label, mesh_paths) in enumerate(
        ...
        with torch.no_grad():
            loss, _ = net(feats, centers, cordinates)
...
if __name__ == '__main__':
    ...
    if 'ShapeNet' in dataroot:
        ...
        print(f"test_len:{len(test_dataset)}")#len(train_dataset)
